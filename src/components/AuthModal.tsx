...
export default function AuthModal({ onClose }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Check uniqueness before redirecting
    ...
    // Store the chosen username so the callback can create the profile
    localStorage.setItem("tangie_pending_username", trimmed);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="text-center mb-7">
          <h2 className="text-xl font-extrabold text-navy mb-2">Join Tangie</h2>
          <p className="text-sm text-gray-500">Choose a username, then sign in with Google.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 border border-gray-200 rounded-xl py-3 px-4 text-sm font-semibold text-navy hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {!loading && (
